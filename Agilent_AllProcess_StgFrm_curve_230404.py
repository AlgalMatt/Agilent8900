...
for run in runnames:
    ...
    if sum(contains1d(runinfo['Sample'], ['stgcrl', 'stgcco', 'stglim']))<3:
        continue
...
#################Processing ##################
    ...
    calistyle='Calibration curve'
    
    #calinames=unique(batch_t.loc[brktrows, 'Sample Name'])
    calirows=[]
    
    #Select cali standards
    unique_names=unique(rundf['Sample'])
    ...
    calinames=unique_names[calidefaults]
    #Find them in the sequence and get the index
    calindx_default=rundf['Sample'].isin(calinames)
    ...
    #                             " are selected"))
    calindx=np.array([i for i, x in enumerate(calindx_default) if x])
    ...
    for cali in calinames:    
        associate_default=[x in cali.lower() for x in nlower(stndval_names)]
       ...
       #                                           "standard name".format(cali)))]   
        cal_associate=stndval_names[associate_default]
    ...
    PA_df_cali=rundf.loc[calindx_default, :]
    for iso in isolist:
        idx=PA_df_cali['Isotope gas']==iso 
        tempdf=PA_df_cali.loc[idx, :]     
        PAcounts=tempdf['PA'].value_counts()
        idx2=tempdf['PA']!=PAcounts.index[0]
        PA_minor=tempdf.loc[idx2, 'Sample']
        #PA_minor=PA_df_cali[PA_df_cali[iso]!=PAcounts.index[0]]['Sample Name']
        if PA_minor.shape[0]>0:       
            for st in unique(PA_minor):
    ...
    cali_single_df=runinfo.copy()
    cali_single_se_df=runinfo.copy()
    cali_curve_df=runinfo.copy()
    cali_curve_se_df=runinfo.copy()
    ...
    for i, row in runcps.iterrows():
        ...
        std_s_array=[]
        for iso in cali_isos:
            if isoel_dict[iso]=='Ca':
                std_s_array.append(1)
            else:
                std_s_array.append(calivals_df.loc[iso, 'STGFrm'])
        
        std_s_array=np.array(std_s_array)
        
        
        #calibrate sample to known standard values
        cali_smpl=brkt_smpl_df.loc[i, cali_isos]*std_s_array
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            cali_smpl_se=(brkt_smpl_se_df.loc[i, cali_isos]
                          /brkt_smpl_df.loc[i, cali_isos])*cali_smpl
        cali_single_df.loc[i, isolist]=cali_smpl
        cali_single_se_df.loc[i, isolist]=cali_smpl_se
    
    
    #calibration (cali curve)   
   
    stnd_array=np.empty((0, len(calivals_df)))  
    stnd_df=rundf.loc[calindx_default, :]
    calindx=unique(stnd_df['Runorder'])-1
    
    for c in calindx:
        s_array=np.array(stnd_dict[runinfo.loc[c, 'Sample']]).T
    ...
    params_ls=[]
    params_se_ls=[]
    cali_curve_df=cali_single_df.copy()
    cali_curve_se_df=cali_single_se_df.copy()
    for i, iso in enumerate(cali_isos):
       ...
       #get the bracketed values of the calibration standards
        ...
        stndiso_arr=np.array(stnd_array[:, i])
        
        
       
        
        if np.all(pd.isnull(stndiso_arr)):
            continue
        
        brktiso_arr=brktiso_arr[~pd.isnull(stndiso_arr)]
        brktiso_se_arr=brktiso_se_arr[~pd.isnull(stndiso_arr)]
        stndiso_arr=stndiso_arr[~pd.isnull(stndiso_arr)]
        
        idx=brktiso_arr<0
        brktiso_arr=brktiso_arr[~idx]
        brktiso_se_arr=brktiso_se_arr[~idx]
        stndiso_arr=stndiso_arr[~idx]
        ...
        Y=Y[np.all(~np.isnan(Xwint), axis=1)]
        
        w=w[np.all(~np.isnan(Xwint), axis=1)]
        Xwint=Xwint[np.all(~np.isnan(Xwint), axis=1), :]
        ...
        if r_sq<0.95:
            #outlier detection
            stgfrmval=stndvals_df.loc[isoel_dict[iso],'STGFrm']
            expectedbrkt=stndiso_arr/stgfrmval
            
            upper=expectedbrkt*1.25
            lower=expectedbrkt*0.75
            
            idx=(upper<brktiso_arr) | (lower>brktiso_arr)
        ...
        params_se_ls.append(params_se)
        #Apply regression to samples.
        cali_curve_df[iso]=brkt_smpl_df[iso]*params[1]+params[0]
        #propagate uncertainty
        cali_curve_se_df[iso]=(((brkt_smpl_se_df[iso]/brkt_smpl_df[iso])**2 
            + (params_se[1]/params[1])**2)*(cali_curve_df[iso]-params[0])**2
                              +params_se[0]**2)**0.5

    r_sq_arr=[]
    for x in r_sq_ls:
        r_sq_arr.append([x]*len(cali_curve_df))
    r_sq_arr=np.array(r_sq_arr)
    r_sq_arr=r_sq_arr.flatten()

    
    params_arr=np.array(params_ls).T
    params_se_arr=np.array(params_se_ls).T
    
    
   
    theo_brkt_rse_df=runinfo.copy()
    theo_cali_curve_rse_df=runinfo.copy()
    for i, row in runcps.iterrows():
        if any(i==blkrows):
            continue
        brkt_u=brkt_r_df.loc[i, 'upper']
        brkt_l=brkt_r_df.loc[i, 'lower']
        theo_x=np.array(theo_R_rsd_df.loc[i, cali_isos])
        theo_s1=np.array(theo_R_rsd_df.loc[brkt_u, cali_isos])
        theo_s2=np.array(theo_R_rsd_df.loc[brkt_l, cali_isos])
        theo_s_mean=np.mean([theo_s1, theo_s2], axis=0)
        theo_brkt_rse_df.loc[i, cali_isos]=(theo_x**2+theo_s_mean**2)**0.5/c4/5**0.5
     
    theo_cali_curve_rse_df[cali_isos]=(((theo_brkt_rse_df[cali_isos]**2+(params_se_arr[1, :]/params_arr[1, :])**2))*(cali_curve_df[cali_isos]-params_arr[0, :])**2+params_se_arr[0, :]**2)**0.5/cali_curve_df[cali_isos]
    ...
    cali_single_melt=cali_single_df.melt(id_vars=idcols, var_name='Isotope gas',
                                     value_name='Cali_single')
    cali_single_se_melt=cali_single_se_df.melt(id_vars=idcols, var_name='Isotope gas',
                                     value_name='Cali_single_se')
    cali_curve_melt=cali_curve_df.melt(id_vars=idcols, var_name='Isotope gas',
                                     value_name='Cali_curve')
    cali_curve_se_melt=cali_curve_se_df.melt(id_vars=idcols, var_name='Isotope gas',
                                     value_name='Cali_curve_se')
    ...
    theo_single_rse_melt=theo_brkt_rse_df.melt(id_vars=idcols, var_name='Isotope gas',
                                     value_name='theo_single_rse')
    theo_curve_rse_melt=theo_cali_curve_rse_df.melt(id_vars=idcols, var_name='Isotope gas',
                                     value_name='theo_curve_rse')
    ...
    rundf=pd.merge(rundf, cali_single_melt)
    rundf=pd.merge(rundf, cali_single_se_melt)
    rundf=pd.merge(rundf, cali_curve_melt)
    rundf=pd.merge(rundf, cali_curve_se_melt)
    ...
    rundf=pd.merge(rundf, theo_single_rse_melt)
    rundf=pd.merge(rundf, theo_curve_rse_melt)
    rundf['r_sq']=r_sq_arr
...
big_df.to_csv('big_stgfrm_singleANDcurve_df.csv')
